        ...
        if tuple(node.state) not in explored:
            ...
            for child in node.expand(problem):
                ...
                if tuple(child.state) not in explored and child not in frontier:
                    if problem.goal_test(child.state):
                        #return goal node and explore state
                        return child, list(explored)
                    frontier.append(child)
    return None, None
    ...
    while frontier:
        ...
        for child in node.expand(problem):
            if tuple(child.state) not in explored and child not in frontier:
                ...
                existing = next(item for value, item in frontier.heap if item == child)
                if f(child) < f(existing):
                    #delete if the new one is lower
