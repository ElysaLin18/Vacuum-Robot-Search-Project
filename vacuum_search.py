...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        state2 via action, assuming it costs curNode.path_cost to get up to state1. For our problem state is (x, y) coordinate pair. 
        ...
        cost_turn = self.computeTurnCost(curNode.action, action)
        all_cost = cost
        
        if self.env.costFunc == 'StepTurn':
            all_cost += cost_turn
        elif self.env.costFunc == 'StayUp' or self.env.costFunc == 'StayTop':
            all_cost += ((self.env.height - state2[1]) / self.env.height)
        elif self.env.costFunc == 'StayLeft':
            all_cost += state2[0] / self.env.width

        return all_cost

    def computeTurnCost(self, action1, action2):
        """Compute the cost of turning from action1 to action2."""
        if action1 == action2:
            return 0
        # if 180 degree turn
        if (action1 == 'LEFT' and action2 == 'RIGHT') or \
        (action1 == 'RIGHT' and action2 == 'LEFT') or \
        (action1 == 'UP' and action2 == 'DOWN') or \
        (action1 == 'DOWN' and action2 == 'UP'):
            return 1
    ...
    def findMinManhattanDist(self, pos):
        ...
        for room in self.env.dirtyRooms:
            dist = distance_manhattan(pos, room)
            #replace min
            if dist < minDist:
                minDist = dist
        return minDist
    ...
    def findMinEuclidDist(self, pos):
        ...
        #go through rooms
        for room in self.env.dirtyRooms:
            dist = distance_squared(pos, room)
            #update min
            if dist < minDist:
                minDist = dist
        return minDist
    ...
    def h(self, node):
        ...
        #check euclid or manhattan
        if args['heuristic'] == 'Euclid':
            heur = self.findMinEuclidDist(position)
        else:
            heur = self.findMinManhattanDist(position)
